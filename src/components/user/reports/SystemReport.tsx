...
export const SystemReport = () => {
  ...
  const totalPresencas = attendanceData?.reduce((acc, curr) => 
    acc + curr.presente, 0);
  
  const totalRegistros = attendanceData?.reduce((acc, curr) => 
    acc + curr.presente + curr.falta + curr.atrasado + curr.justificado, 0);

  const averageAttendance = totalRegistros > 0 
